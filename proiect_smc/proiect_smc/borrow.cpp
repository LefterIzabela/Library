    ...
    : QDialog(parent)
{
	...
	if (books_database.open())
	{
		...
		verify_returningdate_query->prepare("select returning_date from borrow where username='ana_maria';");
		...
		while (verify_returningdate_query->next())
		{
			...
			{
				QString late = "YES";
				modify_late_query->prepare("UPDATE users SET late = :late where username = 'ana_maria';");
